                  ...
                  int nrhs, const mxArray *prhs[] )
{
	...
	unsigned int X,Y;
    double *scale,*sigma_scale,*quant,*ang_th,*eps,*density_th,*max_grad;
	double *M,*MP;
	double *imgptr;
    int n_bins;
    ...
    scale = (double *) mxGetData(prhs[1]);
    sigma_scale = (double *) mxGetData(prhs[2]);
    quant = (double *) mxGetData(prhs[3]);
    ang_th = (double *) mxGetData(prhs[4]);
    eps = (double *) mxGetData(prhs[5]);
    density_th = (double *) mxGetData(prhs[6]);
    max_grad = (double *) mxGetData(prhs[7]);
    n_bins = 1024;
	...
	for(i=0; i<m; ++i)
		for(j=0; j<n; ++j)
			*imgptr++ = M[IDX(j,i,m)];
	out = LineSegmentDetection( image, *scale, *sigma_scale, *quant, *ang_th, *eps,
