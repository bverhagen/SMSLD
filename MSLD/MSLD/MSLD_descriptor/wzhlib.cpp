...
bool SaveDescriptor(float * pDes, int nLineCount, byte * pByValidFlag, int * szCountForEachLine, char * txtFilename) {
	FILE * fp1;
	fp1 = fopen(txtFilename,"w");
	...
	// Print if valid for synching purposes.
	for(int i = 0; i < nLineCount; i++) {
		fprintf(fp1,"%i  ", (int) pByValidFlag[i]);
	}
	fprintf(fp1,"\n");

	// Print count for each line
	for(int i = 0; i < nLineCount; i++) {
		fprintf(fp1,"%d  ", szCountForEachLine[i]);
	}
	fprintf(fp1,"\n");

	// Print descriptor for each line
	for(int i = 0; i < nLineCount; i++) {
