...
int loadDescriptor(float * pDes, byte * pByValidFlag, int * szCountForEachLine, const char * txtDescriptor) {
	FILE * fp;
	fp = fopen(txtDescriptor,"r");
	if (! fp) {
		return -1;
	}

	int nLineCount;
	fscanf(fp,"%i",&nLineCount);

	// Fill pByValidFlag with 1 (valid)
	for (int i = 0; i < nLineCount; i++) {
		int tmp;
		fscanf(fp,"%i",&tmp);
		pByValidFlag[i] = (byte) tmp;
	}

	// Fill line counts
	for (int i = 0; i < nLineCount; i++) {
		fscanf(fp,"%i", &szCountForEachLine[i]);
	}

	// Fill pDes
	for (int i = 0; i < nLineCount; i++) {
		// Written in two for-loops for comprehensability
		for(int k = 0; k < nMaxRegionNum*8; k++)
		{
				fscanf(fp,"%f", &pDes[i*nDesDim+k]);
		}
	}
	fclose(fp);
	return nLineCount;
